...
bool ShapesApp::Initialize()
{
	...
	// 修改 mRootSignature, 包含两个cbvTable根参数,分别对应寄存器插槽 0,1. 
	// cbuffer cbPerObject: register(b0), cbuffer cbPass: register(b1)
	BuildRootSignature(); 
	// 修改了 mShaders, mInputLayout,顶点插槽是0
	BuildShadersAndInputLayout(); 
	// 修改 mGeometries[geo->Name],将几个几何体的顶点和索引合并传到GPU,记录了几个图形的顶点/索引偏移, 
	// geo->VertexBufferGPU,在CPU也有内存副本 geo->VertexBufferCPU
	BuildShapeGeometry();
	...
	BuildRenderItems(); 
	// 修改 mFrameResources,指明了 passCount, objectCount
	BuildFrameResources(); 
	// 描述符堆 mCbvHeap 重要的属性有两个: 
	// 1,NumDescriptors = (objCount + 1)*gNumFrameResources(每个物体和渲染过程), 
	// 2,Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV
	BuildDescriptorHeaps(); 
	// 改的还是 cbvHeap,每个帧资源中的每一个物体都需要一个对应的CBV描述符,将物体的常量缓冲区地址和偏移后的句柄绑定,	
	// 在描述符堆中的句柄偏移 (frameIndex * objCount + i) * mCbvSrvUavDescriptorSize  
	// 物体的常量缓存地址偏移 i * sizeof(ObjectConstants)
