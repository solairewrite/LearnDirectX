...
#if defined(DEBUG) || defined(_DEBUG) // DEBUG版本执行,Released版本不执行
...
protected:
	...
	D3DApp(HINSTANCE hInstance); // HINSTANCE是'句柄型'数据类型,相当于装入到了内存的资源ID.句柄实际上是无符号长整数
...
public:
	...
	HINSTANCE AppInst() const; // 简单的存取函数,返回应用程序实例句柄
	HWND MainWnd() const; // 简单的存取函数,返回主窗口句柄 HWND:窗口句柄
	float AspectRatio() const; // 后台缓冲区的宽高比

	bool Get4xMsaaState() const; // 是否开启4X MSAA(多重采样抗锯齿)
	void Set4xMsaaState(bool value);

	int Run(); // 封装应用程序的消息循环,使用Win32的PeekMessage函数,当没有窗口消息时,就会处理游戏逻辑(4.4.3)
...
protected:
	...
	bool InitMainWindow(); // 初始化应用程序主窗口,需要熟悉基本的Win32窗口初始化流程
	bool InitDirect3D(); // 初始化Direct3D(4.3)
	void CreateCommandObjects(); // 创建命令队列(4.3.4),命令列表分配器和命令列表
	void CreateSwapChain(); // 创建交换链(4.3.5)
	...
	void CalculateFrameStats(); // 计算每秒的平均帧数,每帧平均的毫秒数(4.5.4)

	void LogAdapters(); // 枚举系统中所有的适配器(显卡)(4.1.10)
	void LogAdapterOutputs(IDXGIAdapter* asapter); // 枚举指定适配器的全部显示输出(显示器)(4.1.10)
	void LogOutputDisplayModes(IDXGIOutput* output, DXGI_FORMAT format); // 枚举某个显示输出对特定格式支持的所有显示模式(4.1.10)
...
protected:
	...
	// 继承类应该在构造器中设置这些值
	std::wstring mMainWndCaption = L"d3d App"; // Caption:说明文字,wstring:宽字符
